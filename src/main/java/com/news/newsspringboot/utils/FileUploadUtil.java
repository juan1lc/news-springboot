...
public class FileUploadUtil {
     ...
     */
    private static final String FILE_URL_PREFIX = "/images/post/";
    /**
     * 头像保存的位置后缀，也是URL路径后缀(例如：images/photo/43a97094-b478-4149-9bff-772085d568a0.jpg)
     */
    private static final String Photo_URL_SAVE_PATH = "/images/photos/";
    ...
    public static String singleFileUpload(MultipartFile file){
        ...
        log.info("#### 文件上传工具，文件原始名称：originalFilename={}，文件后缀名：fileSuffix={}",originalFilename,fileSuffix);
        // 为了防止文件覆盖的情况，采用UUID作为前缀
        String fileName = Photo_URL_SAVE_PATH + UUID.randomUUID().toString() + fileSuffix;
        log.info("#### 文件上传工具，：生成的文件名：fileName={}",fileName);
        try {
            // 创建文件夹
            File parentPath = new File(FILE_PATH);
